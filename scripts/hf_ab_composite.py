# -*- coding: utf-8 -*-
"""hf 批次组合层 A/B：R4 幸存 hf 合成 vs PROD 复现（月频 top100 等权）
=====================================================================
评价框架组合层终审（research/factor_eval_dimensions_20260912.md §6），
全部无锁（因子湖 + mirror/kline_daily + L0 方向）：

  四臂（月末建仓 top100 等权，持有到下月末，2022-01~2026-09）：
    HF5   = R4 幸存 5 因子（dsem/amihud/vampp/rfirst30/rku）等权 rank-z
    PROD  = size + amihud_20 等权 rank-z（生产口径复现）
    COMB6 = size + amihud_20 + HF5 五因子 六因子等权 rank-z（融合）
    BASE  = 全池等权月收益（基准）
  方向：按 L0 rank_ic20 符号自动定向（负 IC 反向使用）。
  成本：单边 10/20/30bp，每月净 = 毛 − 2×u×bp（u=top100 月单边换手）。

口径声明：无涨跌停/停牌过滤（此前实证影响 ~0.2%）、复权价、
月末收盘调仓、池=ashare_ex。诚实执行允许负面结论。

用法：python scripts/hf_ab_composite.py
产出：reports/_tmp/hf_ab_composite_summary.csv
      reports/_tmp/hf_ab_composite_monthly.csv
"""
...
OUT_SUM = ROOT / "reports/_tmp/hf_ab_composite_summary.csv"
OUT_MON = ROOT / "reports/_tmp/hf_ab_composite_monthly.csv"
...
def load_signs(factors: list[str]) -> dict[str, float]:
...
def rank_z_score(me: pd.DataFrame, factors: list[str],
    ...
    等权 = 对当月该股有值因子的 signed z 求均值（skipna 语义）。"""
...
def main() -> None:
    t0 = time.time()
    allf = sorted(set(HF5 + PRODF))
    me0 = CON.execute(f"""
        SELECT date FROM read_parquet(
            '{(FACTOR_DIR / HF5[0]).as_posix()}/*.parquet')""").fetchdf()["date"]
    me_list = month_end_dates(me0)
    print(f"月末 {len(me_list)} 个: {me_list[0].date()} ~ {me_list[-1].date()}",
          flush=True)

    print("因子方向（L0 rank_ic20 符号）...", flush=True)
    signs = load_signs(allf)
    print(signs, flush=True)
    ...
    # ---------- 四臂合成
    arms = {}
    arms["HF5"] = rank_z_score(me, HF5, signs)
    arms["PROD"] = rank_z_score(me, PRODF, signs)
    arms["COMB6"] = rank_z_score(me, sorted(set(HF5 + PRODF)), signs)
    ...
    rows = []
    for name in ["HF5", "PROD", "COMB6", "BASE"]:
