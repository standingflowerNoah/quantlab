...
输出：reports/lgbm_all_20260907.json + factor/lgbm_all_score 湖
"""
import sys
import time
import warnings
...
def main():
    ...
    store.factor_dir("lgbm_all_score").mkdir(parents=True, exist_ok=True)
    out.to_parquet(str(store.factor_dir("lgbm_all_score") / "part-b00001.parquet"),
                   index=False)
    print(f"\nscore 已落湖 lgbm_all_score：{len(out)} 行")
    ics.reset_index().rename(columns={0: "ic"}).to_csv(
        ROOT / "reports" / "lgbm_all_oos_ic.csv", index=False)
    imp.head(50).to_csv(ROOT / "reports" / "lgbm_all_importance.csv")
